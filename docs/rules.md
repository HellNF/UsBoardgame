...
- **Usare un oggetto attivo** (`USE_ITEM`): al massimo uno per turno. Sono attivi: Dado singolo, Dado truccato,
  Scala portatile, Ladro, Scambio. Antidoto e Salta domanda sono reattivi (vedi Oggetti).
- **Tirare** (`ROLL`).
...
- **Testa di un serpente:**
  ...
  - altrimenti **sfida lampo** di [`challenges.snakeFlashSeconds` = 30] s (carta con `snakeFlash: true`):
    se il giocatore vince resta dov'è (+1 sfida vinta, nessuna moneta), altrimenti scende alla coda.
...
penalità (e senza salita, se era per una scala).
...
- **Timer:** il server fissa `deadlineAt`; alla scadenza di una prova senza verdetto la prova è fallita; di un
  duello si passa alla doppia conferma.
...
- **Monete doppie:** ogni **guadagno** ottenuto mentre si è in una casella ≥ [`doubleCoinsFrom` = 71] vale doppio
  (anche premi di sfida e monete delle domande). Le perdite non raddoppiano.
...
- Quando un giocatore raggiunge o supera la 100 (con il tiro o con uno spostamento speciale) si segna
  `finishedAtRound`. Il primo in ordine di turno prende [`stars.finishBonus` = 3] stelle.
